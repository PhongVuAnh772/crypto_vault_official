...
const useUnAddedNFTTab = ({ navigation }: RootNavigationType) => {
  ...
  const cursorCollectionsMoralis = useAppSelector(getCursorCollectionsMoralis);
      ...
      if (protocolSelected && wallet?.address) {
        ...
        if (getCollectionsMoralisThunk.fulfilled.match(response)) {
          if (response.payload.data && "result" in response.payload.data) {
            ...
            dispatch(
              setCursorCollectionsMoralis(response.payload.data.cursor ?? null)
            );
            dispatch(setEnableTonPagination(true));
      ...
      if (getCollectionsMoralisThunk.fulfilled.match(response)) {
          if (response.payload.data && "result" in response.payload.data) {
            ...
            if (response.payload.data.cursor) {
              dispatch(
                setCursorCollectionsMoralis(response.payload.data.cursor)
              );
            } else {
