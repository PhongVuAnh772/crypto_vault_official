...
export const objectToInjection = (
    obj: Record<string, any>,
    timeout: number | null,
) => {
    const funcKeys = Object.keys(obj).filter(
...
export const getSessionData = () => {
  return `
   (function() {
      (function() {
        let allLocalStorage = JSON.stringify(localStorage);
        window.ReactNativeWebView.postMessage(allLocalStorage);
      })();
      true;
  `;
