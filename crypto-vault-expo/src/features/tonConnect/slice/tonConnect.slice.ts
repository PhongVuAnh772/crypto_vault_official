            ...
            if (state.connectedApps[chainName]?.[address]) {
                ...
                if (index !== -1) {
                    ...
                    if (state.connectedApps[chainName][address]?.[hash]) {
                        state.connectedApps[chainName][address][
                            hash
                        ].connections = state.connectedApps[chainName][address][
                            hash
                        ].connections.filter(
                            connection =>
                                connection.type !==
                                TonConnectBridgeType.Injected,
                        );
                        if (
                            state.connectedApps[chainName][address][hash]
                        ...
                        if (
                            Object.keys(state.connectedApps[chainName][address])
                                .length === 0
                        ) {
                            delete state.connectedApps[chainName][address];
                        }
