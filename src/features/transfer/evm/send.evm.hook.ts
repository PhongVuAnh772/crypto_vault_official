...
const useSendEVM = ({ navigation }: RootNavigationType) => {
            ...
            if (gasFee > balanceCoin) {
                setError(prev => {
                    return {
                        ...prev,
                        amount: t(
                            LanguageKey.evm_not_enough_amount_to_cover_gas_fee,
                            {
    ...
    const handleSetMaxAmount = () => {
        Keyboard.dismiss();
        const convertAmount = commonUtils.removeTrailingZeros(
            sendMaximum ?? '',
        );
    ...
    const handleApproveToken = async (pinCode: string) => {
        try {
            ...
            const { commissionContractAddress, tokenSelected, path, protocol } =
                data;
    ...
    const handleInitData = async () => {
        ...
        try {
            ...
            if (tokenSelected.isNativeToken) {
                ...
                const feeAmount =
                    (+convertBalanceFollowDecimal * protocol.coinTransferFee) /
                    100;
                const convertFeeToBigInt =
                    commonUtils.convertAmountToWeiFollowDecimals(
                        feeAmount,
                        decimals,
                    );
    ...
    const amountFollowCurrency = (amount: string) => {
        if (cryptosCurrency && selectedCurrencySetting) {
            ...
            const amountCurrency =
                Number(commonUtils.keepNumbers(amount)) * rate;
            const formatCurrency =
                commonUtils.formattedCurrency(amountCurrency);
