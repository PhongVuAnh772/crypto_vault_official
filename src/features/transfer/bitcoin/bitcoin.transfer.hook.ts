...
export const useBitcoinTransfer = ({ navigation }: RootNavigationType) => {
    ...
    const bitcoinTransactionData = useAppSelector(
        selectorBitcoinTransactionData,
    );
        ...
        if (pushBitcoinTransactionAction.fulfilled.match(result)) {
            ...
            if (payloadData && bitcoinTransactionData) {
                const data: TransactionHistoryDataType = {
                    ...
                    adminFee: parseInt(
                        bitcoinTransactionData.adminFee.toString(),
                        10,
                    ),
    ...
    const getSubErrorTitle = (): {
        ...
        if (pushBitcoinHashErrorExists) {
            return {
                ...
                subTitleWithI18N:
                    LanguageKey.push_bitcoin_hash_error_exists_sub_title,
            };
        }
        if (pushBitcoinHashErrorDust) {
            return {
                ...
                subTitleWithI18N:
                    LanguageKey.push_bitcoin_hash_error_dust_sub_title,
            };
        }
        return {
    ...
    const isPushTransactionFailure = useAppSelector(
        getIsPushTransactionFailure,
    );
            ...
            dispatch(
                createBitcoinTransaction({
                    ...
                    amountSend: parseFloat(
                        BitcoinUtils.getSatoshiFromBitcoin(
                            amountSend,
                        ).toString(),
                    ),
                    adminAddress:
