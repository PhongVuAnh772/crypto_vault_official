...
export const useProjectDetail = ({
  ...
  const getDataClaim = async () => {
    ...
    try {
      ...
      if (handleClaimTokenSpecified.fulfilled.match(res)) {
        ...
        if (!result || !result?.project?._id || !currentWallet?.address) {
          navigation.goBack();
          Utils.showToast({
        ...
        if (handleGetInformationLinkTon.rejected.match(resultLinkingTon)) {
          navigation.goBack();
          Utils.showToast({
            msg: t(LanguageKey.claim_data_error_status),
            type: AppToastType.error,
  ...
  const handleActionSwitching = (promisingProtocol?: string) => {
    ...
    const promisingProtocolSelecting =
      protocolListsWithSupportedTokensFromBE?.filter(
        (item) => item.slip0044 === dataClaimable?.projectNftProtocol?.slip0044
      );
    if (promisingProtocolSelecting && promisingProtocolSelecting.length > 0) {
  ...
  const pickingActionWhatYouGot = () => {
    if (
      protocolBaseData?.slip0044 === dataClaimable?.projectNftProtocol?.slip0044
    ) {
      navigation.dispatch(StackActions.push(HomeStackScreenKey.NFTYouOwnList));
    } else {
