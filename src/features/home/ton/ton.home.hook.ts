...
const useTonHome = ({ navigation }: RootNavigationType) => {
    ...
    const getActiveOtherTokenInLocal = (contractAddress: string) => {
        return listToken.find(
            (
                ...
                return (
                    e.active === true &&
                    newData.contractAddress === contractAddress
                );
            },
        );
    };
    ...
    const createCryptoData = async (
        ...
        const finalProtocolBaseData =
            currentProtocolBaseData ?? protocolBaseData;
        ...
        if (otherToken) {
            ...
            for await (const e of otherToken) {
                const activeTokenLocal = getActiveOtherTokenInLocal(
                    e.contractAddress,
                );
                if (activeTokenLocal) {
    ...
    const createCryptoItemData = async (
        ...
        if (tokenData?.isNativeToken) {
            ...
            const redxTokenParsed = TonUtils.getRawAddress(
                appTokens.REDX_TOKEN,
            );
    ...
    const updateTotalBalance = (
        currentListCryptoData: ListCryptoDataType[],
    ) => {
