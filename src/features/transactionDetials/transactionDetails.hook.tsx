...
const useTransactionDetails = ({ navigation }: RootNavigationType) => {
  ...
  const getTonFinalAmount = (
    ...
    if (isNative) {
      return `${TonUtils.formatTonBalance(amountWithDecimal)} ${finalSymbol}`;
    } else {
  ...
  const handleFeeValueFlow = (detail: EventDetail | null) => {
    ...
    return {
      fee: fee?.fees,
      totalChange: fee?.ton,
      ton_attached: isCalledContractTx?.SmartContractExec.ton_attached ?? 0,
    };
  };
  ...
  const convertTitle = ({
    ...
    const getSymbol = () => {
      if (gasFee || isSendEVMNFT) return transactionData.protocolData?.symbol;
      return (
        transactionData?.tokenSymbol ?? transactionData.protocolData?.symbol
      );
    };
    ...
    switch (transactionData.coinType) {
      ...
      case CoinType.Ton:
        ...
        return getTonFinalAmount(
          ...
          finalDecimal,
          finalSymbol,
          isNetworkFee === true ? isNetworkFee : transactionData.isNative
        );
  ...
  if (isTonNFTTransfer) {
    ...
    if (isReceiveTonNFT) {
      ...
      const nftFee = transactionData.fee
        ? convertNFTFeeTon((transactionData.fee + adminFee || 0).toString())
        : "";
  ...
  useEffect(() => {
    const getDataNFTTonTransfer = async () => {
      ...
      try {
        if (currentProtocol && currentProtocol.slip0044 === Slip0044.Ton) {
          ...
          if (getDetailTransactionResult) {
            ...
            const transactionCallSmartContract =
              handleTransactionCallSmartContract(getDetailTransactionResult);

            setNetworkFeeTon(networkFeeValueFlow ?? null);
            setIsTransactionCallSmartContract(transactionCallSmartContract);
          }
        }
        return;
      } catch (error) {
