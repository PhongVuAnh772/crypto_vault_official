    ...
    reducers: {
        ...
        activeJettonFromBE: (
            ...
            if (state.listTokenFromBE.hasOwnProperty(id)) {
                state.listTokenFromBE[id] = state.listTokenFromBE[id].map(
                    item => {
        ...
        onChangeActiveJetton: (
            ...
            if (state.listTokenByProtocol.hasOwnProperty(id)) {
                state.listTokenByProtocol[id] = state.listTokenByProtocol[
                    id
                ].map(item => {
            ...
            if (state.listTokenFromBE.hasOwnProperty(id)) {
                state.listTokenFromBE[id] = state.listTokenFromBE[id].map(
                    item => {
                        if (!item.isNativeToken) {
                            const token =
                                item as SupportedTokenItemWithProtocol;
        ...
        convertTokenFromBE: (
            ...
            if (data) {
                const allTokens = data.flatMap(protocol => {
                    if (
                        !(
                            protocol.VM === VMType.EVM ||
                            protocol.VM === VMType.Ton
                        )
                    ) {
                        return [];
                    }
                    return protocol.supportedToken.map(token => {
                        const convertedTokenBE =
                            token as SupportedTokenItemWithProtocol;

                        const findItemSaved = state.listTokenFromBE?.[
                            protocol._id
                        ]?.find(tokenLocal => {
                        ...
                        const data: TokenType = {
                            ...
                            balanceCurrency:
                                findItemSaved?.balanceCurrency ?? 0,
                        };
                            ...
                            for (const key in state.listTokenByProtocol) {
                                ...
                                if (parseInt(slip, 10) === protocol.slip0044) {
                                    const filteredTokens =
                                        state.listTokenByProtocol[key].filter(
                                            tokenLocal => {
        ...
        updateBalanceToken: (
            ...
            state.listTokenByProtocol[id] = state.listTokenByProtocol[id]?.map(
                token => {
                    ...
                    if (tokens[lowerCaseToken]) {
                        token.balance = +tokens[lowerCaseToken].balance;
                        token.balanceCurrency =
                            tokens[lowerCaseToken].usd_price;
                    }
                    return token;
                },
            );
...
export const filterTokenAvailable = createSelector(
    ...
    (data, listTokenByProtocol, listTokenFromBE) => {
        ...
        const filterListBETokens = backendTokens
            ...
            .sort(
                (a, b) => (b.isNativeToken ? 1 : 0) - (a.isNativeToken ? 1 : 0),
            );
