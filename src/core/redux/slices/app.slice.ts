...
import { DefaultProtocolDataList } from "assets/mock/DefaultProtocolData";
...
} from "src/core/redux/slices/nft.slice";
...
import { convertTokenFromBE } from "./token.slice";
...
const initialState: AccountSliceType = {
  ...
  protocolListsWithSupportedTokensFromBE: undefined,
...
export const getMobileProtocolListsWithSupportedTokens = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const accountState = state?.account as any;
...
export const addAccount = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const selectedProtocolId = rootState.account.selectedProtocolId;
      const accountLists = rootState.account.accountLists;
      ...
      if (!protocolListsWithSupportedTokensFromBE) {
        const res = await dispatch(
          getMobileProtocolListsWithSupportedTokens(
            accountLists ? undefined : mnemonic
          )
        );
        if (getMobileProtocolListsWithSupportedTokens.fulfilled.match(res)) {
          protocolListsWithSupportedTokensFromBE = res.payload;
        }
      }
      ...
      const protocolData = await AccountUtils.generateProtocolData({
        mnemonic,
        generateDataProtocolListsFromBE: protocolListsFromBE as any,
        dispatch,
      });
