import { sendGet, sendPost } from "src/network/requests";
import {
  APIResponseMoralis,
...
import { CollectionDetailMoralisResponse } from "../redux/type/nft.type";
...
class MoralisService {
  private async handleError<T>(result: T): Promise<void> {
    ...
    if (status !== 200 || (data && "message" in data)) {
      console.error("MoralisService Error:", data);
    }
  }
