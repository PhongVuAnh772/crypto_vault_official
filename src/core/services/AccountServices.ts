import { AccountType } from "src/type/AccountType";
import AccountUtils from "src/utils/accountUtils";
import SecureStorageKey from "../enum/SecureStorageKey";
...
class AccountServices {
  ...
  saveAccountDataWithEncrypt = async (
    ...
    try {
      ...
      if (checkAccountData) {
        ...
        await SecureStorage.setObject(SecureStorageKey.accounts, encryptData);
        return true;
      } else {
        return false;
      }
  ...
  getCurrentAccount = async (
    ...
    try {
      ...
      if (this.encryptAES.isAccountEncryptedType(accountEncryptedData)) {
        const encryptAES = new EncryptAES();
        const accountsData: AccountType[] = await encryptAES.decryptObject(
          accountEncryptedData,
          pinCode
        );
      ...
      console.log("🚀 ~ AccountServices ~ getCurrentAccount ~ error:", error);
    }
  };
}
