...
import TonConnectUtils, {
  ConnectEventError,
} from "src/utils/TonConnectNativeUtils";
import {
  IConnectedApp,
...
import TonConnectTransfer from "./TonConnectTransfer";
...
class TonConnectService {
  ...
  async sendTransaction(
    ...
    try {
      ...
      const transactionData = await tonTransaction.createTransferTonConnect({
        privateKey: privateKey,
        version: version,
        publicKey: publicKey,
        estimateFee: true,
        fromAccountData: finalFromAccountData,
        param: params.messages as any,
      ...
      if (transactionData) {
        if (isDummySecretKey) {
          ...
          return {
            result: transactionData?.transferData.messageBOCString ?? "",
            id: request.id,
          };
        }
      } else {
