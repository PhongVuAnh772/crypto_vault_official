...
class Web3Service {
    ...
    async getOwnerOfNFT({
        ...
        try {
            const { contractABI } =
                this.instantiateASmartContract(nftSmartContract);

            const ownerAddress = await contractABI.methods
                ?.ownerOf(tokenId)
                ?.call();
    ...
    async getApproveNFTERC721({
        ...
        try {
            ...
            const getApproved = await contractABI.methods
                .getApproved(tokenId)
                .call();
    ...
    async approveNFTERC721({
        ...
        try {
            ...
            const data = await this.getPrivateKeyAndNonceAddress(
                pinCode,
                path,
                slip,
            );
    ...
    async transferNativeToken({
        ...
        try {
            ...
            const totalValue = (BigInt(amount) + BigInt(commission)).toString();

            const contractTransfer = contractABI.methods.transferNativeToken(
                recipientAddress,
                amount,
                beneficiaryAddress,
                commission,
    ...
    async estimateGasTransferNativeToken({
        ...
        commission,
        sender,
    }: Omit<TransferNativeTokenParamsType, 'pinCode' | 'path' | 'slip'> & {
        sender: string;
    }): Promise<bigint> {
        try {
            const { contractABI } = this.instantiateASmartContract(
            ...
            const [estimateGas, gasPrice] = await Promise.all([
                contractABI.methods
                    ...
                    .estimateGas({
                        from: sender,
                        value: (BigInt(amount) + BigInt(commission)).toString(),
                    }),
                this.getCurrentGasPrice(),
            ]);
            if (!estimateGas || !gasPrice) {
    ...
    async processCheckNFT({
        ...
        if (typeof owner === 'undefined') {
            ...
            if (convertedBalance > 0) {
                ...
                throw new Error(
                    t(LanguageKey.error_you_are_not_owner_of_this_nft),
                );
            }
        } else if (
            convertedOwner.toLowerCase() !== walletAddress.toLowerCase()
        ) {
    ...
    async estimateGasForNormalNativeTransfer(
        ...
        const tx = {
            from,
            to,
            value: commonUtils.convertAmountToWeiFollowDecimals(
                amount,
                decimals,
            ),
        };
    ...
    async transferNativeTokenNormal(
        ...
        try {
            ...
            const [{ estimateGas }, gasPrice] = await Promise.all([
                this.estimateGasForNormalNativeTransfer(
                    from,
                    to,
                    amount,
                    decimals,
    ...
    async transferTokenNormal(
        ...
        try {
            ...
            const convertedAmount =
                commonUtils.convertAmountToWeiFollowDecimals(amount, decimals);
