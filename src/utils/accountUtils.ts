...
import TonWallet from "src/core/services/TonWalletService";
import {
  AccountSliceType,
  AccountType,
  AddressListItemType,
...
const getTonAddress = async (version: any, publicKey: string) => {
...
const generateProtocolData = async ({
  ...
  for (const protocol of generateDataProtocolListsFromBE) {
    ...
    if (slip0044 != null) {
      walletCoreCoinData = await nativeWalletCoreModule.getDataFromSlip0044({
        mnemonic,
        isTestNet,
        slip0044,
        derivationPath,
    ...
    if (slip0044 != null) {
      ...
      if (beneficiaryWalletAddress && beneficiaryStatus === "approved") {
        ...
        if (slip0044 === Slip0044.Bitcoin) {
          const resCheckBitcoinAddress = await BitcoinUtils.isValidAddress(
        ...
        if (slip0044 === Slip0044.Ethereum || slip0044 === Slip0044.Polygon) {
          isCheckBeneficiaryWalletAddress = true;
        }
      ...
      account.protocolData.some(
        (protocol) =>
          ...
          protocol.addressList.some(
            (addr) =>
              Address.parse(addr.address).toRawString() === addressRequest
          )
      )
    );
  ...
  if (coinProtocolData && allCount) {
    const account = allCount.find((account) =>
      account.protocolData.some((protocol) =>
        protocol.addressList.some(
          (addr) =>
            addr.address.toUpperCase() === addressRequest.toUpperCase() &&
            protocol._id?.includes(coinProtocolData?._id)
        )
      )
    );
