...
import { ImportNFTTonParams, NFTTonData } from "src/core/redux/type/nft.type";
import {
  NftTonNewItem,
  TonEvent,
  TonEventsAction,
...
import DateTimeUtils from "src/type/DateTimeUtils";
import { TransactionHistoryDataType } from "type/History";
import { BeneficiaryType } from "type/ProtocolType";
import { SettingCurrencyType } from "type/SettingType";
...
const _handleMultipleActions = async ({
  ...
  try {
      ...
      if (dataTransfer) {
        ...
        const isAdminAction =
          dataTransfer.recipient && beneficiary?.walletAddress
            ? action.status === "ok" &&
              (dataTransfer?.comment?.toLocaleLowerCase() === "admin fee" ||
                Address.parse(dataTransfer.recipient.address).toString() ===
                  Address.parse(beneficiary?.walletAddress ?? "").toString())
            : false;
    ...
    if (multipleTransactionData[0] && multipleTransactionData[0]?.isAdminFee) {
...
const _handleOnlyAction = async ({
  ...
  const tonTransactionHistoryDataItem: TransactionHistoryDataType = {
    ...
    amountSend: amount ?? 0,
    type: transactionType,
    tokenSymbol: isJettonAction ? dataTransfer?.jetton?.symbol : customSymbol,
    tokenTransferType: isJettonAction ? action.type : undefined,
    decimal: isJettonAction ? dataTransfer?.jetton?.decimals : decimal,
    nativeDecimal,
    logoUri: isJettonAction ? dataTransfer?.jetton?.image : undefined,
...
const checkExists = (
  ...
  if (nft.tonCollection === undefined) {
    return false;
  }
  if (params.id && nft.tonCollection[params.accountId]) {
    const findCollection = nft.tonCollection[params.accountId]?.[
      params.id
    ]?.find(
    ...
    if (findCollection) {
      ...
      try {
        const nftExists = data?.some((item) => {
          const convertedAddress = Address.parse(item.root.contractAddress);
          return (
