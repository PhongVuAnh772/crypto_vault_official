...
import { TonAccountsType } from "src/navigation/stack/type/TonServiceType";
import { AddressListItemType } from "type/ProtocolType";
import TonConnectService from "src/core/services/TonConnect";
...
const parsePayload = (payload?: string, customExcessesAccount?: string) => {
  ...
  if (customExcessesAccount) {
    body = rebuildBodyWithCustomExcessesAccount(body, customExcessesAccount);
  }
  return body;
};
...
const internalMessages = async (
  ...
  try {
    for (const element of param) {
      const recipientAccountData = await getReceiveAddressData(element.address);
