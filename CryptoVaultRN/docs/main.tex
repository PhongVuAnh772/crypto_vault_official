...
\usepackage{fancyhdr}
\usepackage{tikz}
\usetikzlibrary{positioning,shapes,arrows,babel}
...
\begin{titlepage}
    \begin{center}
        ...
        \textit{o0o}
        \vspace{2cm}
        \begin{figure}[h] % Changed the positioning of the figure
            \centering
...
\begin{itemize}
    \item \textbf{Người dùng cá nhân sở hữu tiền mã hóa (Crypto Investors/Users):} Những cá nhân có nhu cầu lưu trữ an toàn các tài sản kỹ thuật số như Bitcoin, TON, Ethereum, và các token tiêu chuẩn khác. Họ cần thực hiện các giao dịch gửi và nhận tiền nhanh chóng, bảo mật trên thiết bị di động cá nhân của mình.
    
    \item \textbf{Người thu thập và giao dịch tác phẩm nghệ thuật số (NFT Collectors):} Các cá nhân sở hữu và có nhu cầu hiển thị, quản lý các bộ sưu tập tài sản số độc bản (NFT) trên nhiều mạng lưới blockchain khác nhau.
    
    \item \textbf{Đối tác thứ ba (Partners / Event Organizers / Third-party Providers):} Các doanh nghiệp, nhà tổ chức sự kiện, rạp chiếu phim, hãng hàng không có nhu cầu số hóa vé vào cửa dưới dạng NFT, theo dõi doanh thu soát vé và tích hợp hệ thống phát hành qua API và nhận dữ liệu qua Webhook.
    
    \item \textbf{Nhân viên soát vé (Staff / Ticket Scanners):} Nhân sự vận hành tại các địa điểm diễn ra sự kiện sử dụng công cụ camera quét mã trên thiết bị di động để kiểm tra và xác nhận quyền vào cửa của khách hàng bằng QR động hoặc mã offline.
    
    \item \textbf{Nhà phát triển và Quản trị viên hệ thống (Developers/Administrators):} Những người chịu trách nhiệm giám sát hoạt động của hệ thống, phân tích hiệu năng và hành vi người dùng thông qua các công cụ đo lường (Firebase Analytics), cập nhật và bảo trì các dịch vụ tích hợp bên thứ ba (Moralis, TonServices) để đảm bảo hệ thống vận hành trơn tru.
\end{itemize}

Mỗi đối tượng đều có nhu cầu và mục đích sử dụng riêng biệt, cùng tương tác trên một giao diện ví đồng nhất để tối ưu hóa hiệu quả quản trị tài sản số.
...
\end{figure}
- Biểu đồ phân rã chức năng quản lý ví và bảo mật
\vspace{0.4cm}
...
- Biểu đồ phân rã chức năng giao dịch và chuyển nhận
\vspace{0.4cm}
...
- Biểu đồ phân rã chức năng quản lý tài sản và Web3
\vspace{0.4cm}
...
- Biểu đồ phân rã chức năng cấu hình hệ thống
...
\begin{tikzpicture}[
    actor/.style={draw,circle,minimum size=1cm},
    usecase/.style={draw,ellipse,minimum width=2.8cm,minimum height=1cm},
    >=latex
]

% Actors
\node[actor] (user) at (-6,0) {Người dùng ví};
\node[actor] (admin) at (6,0) {Admin / Dev};
\node[actor] (blockchain) at (0,-5) {Mạng Blockchain};

% Use cases
\node[usecase] (create) at (0,4.5) {Tạo / Khôi phục ví};
\node[usecase] (unlock) at (0,3) {Mở khóa ví};
\node[usecase] (send_receive) at (-2.5,1.2) {Gửi / Nhận tài sản};
\node[usecase] (manage_asset) at (-2.5,-0.8) {Quản lý Token \& NFT};
\node[usecase] (configure) at (-2.5,-2.8) {Cấu hình hệ thống};
\node[usecase] (monitor) at (2.5,1.2) {Giám sát \& Vận hành};
\node[usecase] (maintenance) at (2.5,-0.8) {Bảo trì \& Cập nhật};

% Connections from Wallet User
\draw (user) -- (create);
\draw (user) -- (unlock);
\draw (user) -- (send_receive);
\draw (user) -- (manage_asset);
\draw (user) -- (configure);

% Connections from Admin/Dev
\draw (admin) -- (monitor);
\draw (admin) -- (maintenance);

% Connections from Blockchain
\draw (blockchain) -- (send_receive);
\draw (blockchain) -- (manage_asset);

% System boundary
\draw[dashed] (-4.5,-4.2) rectangle (4.5,5.5) node[above] at (0,5.5) {Hệ thống CryptoVault};

\end{tikzpicture}
\vspace{0.7cm}

 - Usecase tổng quát của toàn ứng dụng sẽ bao gồm các usecase chính sau: Tạo / Khôi phục ví, Mở khóa ví, Quản lý ví và bảo mật, Quản lý tài sản và giao dịch, Cấu hình hệ thống, Giám sát hệ thống.
\vspace{0.7cm}
...
\begin{tikzpicture}[
    actor/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.8cm},
    usecase/.style={draw, ellipse, minimum width=3cm, minimum height=0.8cm},
    include/.style={->, dashed},
]

% Actors on the left
\node[actor] (user) at (-4,0) {Người dùng ví};

% Use cases
\node[usecase] (create) at (0,1.5) {Tạo ví mới};
\node[usecase] (restore) at (0,-1.5) {Khôi phục ví};
\node[usecase] (backup) at (5,3) {Sao lưu Seedphrase};
\node[usecase] (input_seed) at (5,1) {Nhập Seedphrase};
\node[usecase] (setup_pin) at (5.5,-1) {Thiết lập mã PIN};

% Include relationships
\draw[include] (create) -- (backup) node[midway, above] {<<include>>};
\draw[include] (create) -- (setup_pin) node[midway, below] {<<include>>};
\draw[include] (restore) -- (input_seed) node[midway, above] {<<include>>};
\draw[include] (restore) -- (setup_pin) node[midway, below] {<<include>>};

% Associations (solid lines)
\draw (user) -- (create);
\draw (user) -- (restore);

\draw[dashed] (-2.5,-3) rectangle (8.5,4.5) node[above] at (3,4.5) {Tạo / Khôi phục ví};
...
* Đối với Tạo ví mới: Không yêu cầu dữ liệu đầu vào. Hệ thống sẽ tự động sinh Seedphrase ngẫu nhiên.
* Đối với Khôi phục ví: Người dùng cần cung cấp chính xác cụm từ khôi phục 12 hoặc 24 từ tiếng Anh hợp lệ.

\subsubsection{e) Luồng chạy}
\begin{enumerate}
    \item Người dùng khởi động ứng dụng và chọn "Tạo ví mới" hoặc "Khôi phục ví".
    \item Nếu chọn **Tạo ví mới**:
    \begin{itemize}
...
\begin{tikzpicture}[
    actor/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.8cm},
    usecase/.style={draw, ellipse, minimum width=3cm, minimum height=0.8cm},
    extend/.style={->, dashed, red},
    include/.style={->, dashed},
]

% Actors on the left
\node[actor] (user) at (-4,2) {Người dùng ví};

% Use cases
\node[usecase] (unlock) at (1,2) {Mở khóa ví};
\node[usecase] (pin) at (6,3.5) {Xác thực mã PIN};
\node[usecase] (bio) at (6,2) {Xác thực sinh trắc học};
\node[usecase] (forgot) at (6,0.5) {Khôi phục mã PIN};

% Relationships
\draw[include] (unlock) -- (pin) node[midway, above] {<<include>>};
\draw[extend] (unlock) -- (bio) node[midway, below] {<<extend>>};
\draw[extend] (unlock) -- (forgot) node[midway, below] {<<extend>>};

% Associations
\draw (user) -- (unlock);

% Boundary box
\draw[dashed] (-1.5,-0.5) rectangle (8.5,4.8) node[above] at (3.5,4.8) {Mở khóa ví};
...
\begin{tikzpicture}[
    actor/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.8cm},
    usecase/.style={draw, ellipse, minimum width=3.2cm, minimum height=0.8cm},
    extend/.style={->, dashed},
]

% Actors
\node[actor] (user) at (-5,0) {Người dùng ví};
\node[actor] (blockchain) at (9,0) {Mạng Blockchain};

% Main use cases
\node[usecase] (send) at (0,3) {Gửi tài sản};
\node[usecase] (receive) at (0,1.5) {Nhận tài sản};
\node[usecase] (view_balance) at (0,0) {Xem số dư \& Lịch sử};
\node[usecase] (add_token) at (0,-1.5) {Thêm Custom Token};
\node[usecase] (manage_nft) at (0,-3) {Quản lý NFT};

% Extended use cases
\node[usecase] (scan_qr) at (4.5,4) {Quét mã QR ví};
\node[usecase] (calc_gas) at (4.5,2.6) {Tính phí gas ước tính};
\node[usecase] (show_qr) at (4.5,1.2) {Hiển thị QR địa chỉ};
\node[usecase] (input_contract) at (4.5,-0.5) {Nhập Contract Address};
\node[usecase] (view_metadata) at (4.5,-2.2) {Xem metadata NFT};

% Connections from user to main use cases
\draw (user) -- (send);
\draw (user) -- (receive);
\draw (user) -- (view_balance);
\draw (user) -- (add_token);
\draw (user) -- (manage_nft);

% Connections from blockchain to main use cases
\draw (blockchain) -- (send);
\draw (blockchain) -- (view_balance);
\draw (blockchain) -- (add_token);
\draw (blockchain) -- (manage_nft);

% Extend relationships (dashed arrows)
\draw[extend] (send) -- (scan_qr);
\draw[extend] (send) -- (calc_gas);
\draw[extend] (receive) -- (show_qr);
\draw[extend] (add_token) -- (input_contract);
\draw[extend] (manage_nft) -- (view_metadata);

% Boundary box
\draw[dashed] (-3,5) rectangle (7.5,-4.5) node[above] at (2.2,5) {Quản lý tài sản \& Giao dịch};
...
% Actor
\node[actor] (user) at (-3,0) {Người dùng ví};

% Use cases
\node[usecase] (add_token) at (1.5,0) {Thêm Custom Token};
\node[usecase] (query_contract) at (7,0) {Truy vấn Smart Contract};

% Connections
\draw (user) -- (add_token);
\draw[extend] (add_token) -- (query_contract) node[midway, above] {<<include>>};
...
% Main use cases
\node[usecase] (monitor) at (0,2) {Theo dõi ví giám sát};
\node[usecase] (alerts) at (0,0) {Nhận thông báo biến động};
\node[usecase] (threshold) at (0,-2) {Thiết lập cảnh báo};

% Extended use cases
\node[usecase] (view_tx) at (6,2) {Xem chi tiết giao dịch};
\node[usecase] (large_tx) at (6,-1) {Cảnh báo giao dịch lớn};
...
\draw[extend] (monitor) -- (view_tx) node[midway, above] {<<extend>>};
\draw[extend] (alerts) -- (large_tx) node[midway, above] {<<extend>>};

% Boundary box
\draw[dashed] (-2.5,-3.2) rectangle (8.5,3.2) node[above] at (3,3.2) {Giám sát \& Cảnh báo ví};
...
\begin{figure}[h]
    ...
    % Actor
    \node[actor] (user) at (-4,-1) {Người dùng ví};

    % Main use cases
    \node[usecase] (view) at (0,2) {Xem danh sách liên hệ};
    \node[usecase] (add) at (0,0) {Thêm liên hệ mới};
    \node[usecase] (edit) at (0,-2) {Chỉnh sửa liên hệ};
    \node[usecase] (manage) at (0,-4) {Quản lý nhóm/nhãn};

    % Extended use cases
    \node[usecase] (search) at (6,3) {Tìm kiếm liên hệ};
    \node[usecase] (filter) at (6,2) {Lọc theo mạng lưới};
    \node[usecase] (qr) at (6,0) {Quét mã QR địa chỉ};
    \node[usecase] (import_export) at (6,-2) {Xuất/Nhập danh bạ};
    ...
    \draw[extend] (view) -- (search) node[midway, above] {<<extend>>};
    \draw[extend] (view) -- (filter) node[midway, above] {<<extend>>};
    \draw[extend] (add) -- (qr) node[midway, above] {<<include>>};
    \draw[extend] (add) -- (import_export) node[midway, above] {<<extend>>};
    \draw[extend] (edit) -- (qr) node[midway, above] {<<include>>};

    % Boundary box
    \draw[dashed] (-2.8,-5) rectangle (9,4) node[above] at (3.25,4) {Quản lý danh bạ địa chỉ};

    \end{tikzpicture}
    \caption{Mô hình sử dụng cho quản lý danh bạ địa chỉ ví.}
...
\begin{figure}[h]
    \centering
    \label{fig:sequence-restore-wallet}
    \caption{Biểu đồ tuần tự khôi phục ví bằng Seedphrase}
\end{figure}
...
\begin{figure}[h]
    \centering
    \label{fig:sequence-unlock-wallet}
    \caption{Biểu đồ tuần tự mở khóa ví}
\end{figure}

\begin{figure}[h]
    \centering
    \label{fig:sequence-reset-pin}
    \caption{Biểu đồ tuần tự khôi phục mã PIN}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự quản lý bảo mật ví}
    \label{fig:sequence_security}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự thêm Custom Token}
    \label{fig:sequence_add_token}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự quản lý bộ sưu tập NFT}
    \label{fig:sequence_manage_nft}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự thêm ví giám sát}
    \label{fig:sequence_watch_wallet}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự quản lý danh bạ địa chỉ ví}
    \label{fig:sequence_address_book}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự theo dõi biến động và nhận thông báo}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự kết nối dApp qua WalletConnect}
    \label{fig:sequence_dapp_connect}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự ký giao dịch từ dApp}
    \label{fig:sequence_dapp_sign}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự đồng bộ hóa dữ liệu sao lưu đám mây}
    \label{fig:sequence_cloud_sync}
\end{figure}
...
\begin{figure}[h]
    \centering
    \caption{Biểu đồ tuần tự xóa ví / thiết lập lại ứng dụng}
    \label{fig:sequence_delete_wallet}
\end{figure}
...
\begin{figure}[h]
    ...
    \resizebox{0.95\textwidth}{!}{
    \begin{tikzpicture}[
        actor/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, rounded corners=3pt, fill=blue!5},
        usecase/.style={draw, ellipse, minimum width=3.5cm, minimum height=0.9cm, align=center, fill=teal!5},
        extend/.style={->, dashed, >=stealth},
        line/.style={draw, thick}
    ]
    % Actors
    \node[actor] (partner) at (-6, 2) {Đối tác (Partner)};
    \node[actor] (user) at (-6, -2) {Người dùng ví};
    \node[actor] (staff) at (6, -2) {Nhân viên soát vé (Staff)};
    \node[actor] (blockchain) at (6, 2) {Smart Contract};

    % Use Cases
    \node[usecase] (issue) at (0, 3.5) {Phát hành vé \\ (Single/Batch)};
    \node[usecase] (view) at (0, 2) {Quản lý danh \\ sách vé};
    \node[usecase] (transfer) at (0, 0.5) {Chuyển nhượng \\ vé P2P};
    \node[usecase] (show_qr) at (0, -1) {Hiển thị mã \\ QR động};
    \node[usecase] (scan) at (0, -2.5) {Quét soát vé \\ (QR/Offline)};
    \node[usecase] (sync) at (0, -4) {Đồng bộ soát vé \\ ngoại tuyến};

    % Connections
    \draw[line] (partner) -- (issue);
    \draw[line] (partner) -- (view);
    
    \draw[line] (user) -- (view);
    \draw[line] (user) -- (transfer);
    \draw[line] (user) -- (show_qr);
    
    \draw[line] (staff) -- (scan);
    \draw[line] (staff) -- (sync);
    
    \draw[line] (blockchain) -- (issue);
    \draw[line] (blockchain) -- (transfer);
    \draw[line] (blockchain) -- (scan);

    % System boundary
    \draw[dashed] (-3.5, 4.5) rectangle (3.5, -4.8) node[above] at (0, 4.5) {Hệ thống Vé Điện tử (Ticket Platform)};
    \end{tikzpicture}
    }
...
\section{Backend với Express Nodejs}

PostgreSQL là một hệ quản trị cơ sở dữ liệu quan hệ mạnh mẽ và có độ tin cậy cao, được sử dụng phổ biến trong nhiều ứng dụng web nhờ khả năng xử lý giao dịch phức tạp và quản lý dữ liệu dạng JSON linh hoạt. Supabase, mặt khác, là một nền tảng mã nguồn mở, cung cấp các dịch vụ backend như cơ sở dữ liệu, xác thực, lưu trữ và tính năng thời gian thực, và được xây dựng dựa trên PostgreSQL. Khi kết hợp với nhau, PostgreSQL và Supabase tạo thành một giải pháp hiệu quả, linh hoạt và dễ triển khai cho việc xây dựng backend.
...
\subsection{Mô hình RESTful API}

\section{RESTful API và PostgreSQL}
...
\begin{figure}[h] % Changed the positioning of the figure
            \centering
            \label{fig:enter-label}
            \caption{Mô hình hoạt động của RESTful API}
        \end{figure}
...
\section*{Mô tả triển khai Supabase với React Native cho ứng dụng ví điện tử CryptoVault}
...
% Arrows
\draw [arrow] (frontend) -- (auth) node[midway, right] {Auth};
\draw [arrow] (auth) -- (api) node[midway, right] {Token xác thực};
\draw [arrow] (api) -- (db) node[midway, above] {TVDL};
\draw [arrow] (db) -- (api) node[midway, below] {KQTV};
\draw [arrow] (api) -- (realtime) node[midway, above] {CNRT};
\draw [arrow] (realtime) -- (frontend) node[midway, above] {DLTGT};
