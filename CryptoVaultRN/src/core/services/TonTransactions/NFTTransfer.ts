...
import { TransactionNFTEmulateStatusType } from 'src/core/enum/TransactionType';
import TonServices from '../TonServices';
import { EmulateMessageToWalletResType } from '../TonServices/type';
import { TonOpCodes } from './opCode';
import {
...
class NFTTonTransfer {
    ...
    > => {
        try {
            ...
            const { contract, secretKey } =
                await TransferUtils.initializeWallet(
            ...
            const emulateTransferData =
                await tonServices.emulateMessageToWallet({
            ...
            const hasTransferError = actionsTransfer.some(
                action =>
                    action.status === TransactionNFTEmulateStatusType.Failed,
            );
            if (hasTransferError) {
                return undefined;
            }
