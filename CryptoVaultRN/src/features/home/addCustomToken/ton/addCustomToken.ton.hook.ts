...
const useAddCustomToken = ({ navigation }: RootNavigationType) => {
  ...
  const [contractAddress, setContractAddress] = useState<string>("EQC5Dj4AfQaDl40ex6E0WyUDdMBJHbqTXnxNkn8HuQZNzpZt");
  ...
  const handleCheckToken = async (contractAddress: string) => {
    ...
    try {
      ...
      console.log("resDetailJetton", resDetailJetton);
      if (resDetailJetton) {
  ...
  const checkExist = (contractAddress: string) => {
    if (listToken?.length) {
      const listTokenConverted = listToken.map((item) => {
        const addressParsed = Address.parse(item.contractAddress).toRawString();
        return {
          ...item,
  ...
  const handleAddToken = async () => {
    ...
    try {
      ...
      const parsedRawAddress = Address.parse(contractAddress).toRawString();
