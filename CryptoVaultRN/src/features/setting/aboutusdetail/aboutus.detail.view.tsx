...
const AboutUsDetail: React.FC<RootNavigationType> = () => {
  ...
  const { policyContent, lastUpdate, loading } = useAboutUsDetail();
  return (
    <ScreenWrapper
    ...
    >
      <ScrollView
        bounces={false}
        style={[aboutusDetailStyles.h100, aboutusDetailStyles.boxContainer]}
      >
        <View style={aboutusDetailStyles.boxContainer}>
          <View style={aboutusDetailStyles.container}>
            {loading ? (
              ...
              <>
                <AppText
                  titleWithI18n={LanguageKey.setting_last_updated}
                ...
                <View style={aboutusDetailStyles.mt16}>
                  <AppText
                    titleWithI18n={LanguageKey.setting_table_of_content}
                    variant={TextVariantKeys.bodyMMedium}
                  />
                </View>
                <View style={aboutusDetailStyles.mt8}>
                  <AppText
                    titleWithI18n={LanguageKey.setting_privacy_policy}
                    variant={TextVariantKeys.titleSmall}
                    textColor={appColors.main.tokyoRed}
                  />
                  <AppText
                    titleWithI18n={LanguageKey.setting_terms_and_conditions}
                    variant={TextVariantKeys.titleSmall}
                    textColor={appColors.main.tokyoRed}
                  />
                </View>
          ...
          <View style={aboutusDetailStyles.containerPolicy}>
            {loading ? (
              ...
              policyContent?.map((item) => {
                ...
                return <RenderHtml source={htmlSource} key={item._id} />;
              })
            )}
          </View>
        </View>
