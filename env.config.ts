export const ENV_NAME = 'Test';
export const ENV = 'development';

export const API_KEY = 'KGL9WtIo9pbhEIsjZCR1LgOn4armuPINQBs5MSGZAhc6lj';

// MARK: Base url
export const BASE_URL = 'https://cryptovault-backend-latest.onrender.com/api/v1/';

// MARK: Block Cypher API
export const BLOCK_CYPHER_TOKEN = 'c21de897e93d4cc6a4fe623811c21952';
export const BLOCK_CYPHER_BTC_DATA_ULR =
    'https://api.blockcypher.com/v1/btc/test3/addrs/';
export const BLOCK_CYPHER_PUSH_TRANSACTION_ULR =
    'https://api.blockcypher.com/v1/btc/test3/txs/push';
export const BLOCK_CYPHER_PUSH_TRANSACTION_DETAIL_ULR =
    'https://blockstream.info/testnet/tx/';
export const BLOCK_CYPHER_ADDRESS_ULR =
    'https://blockstream.info/testnet/address/';

// MARK: Ton API
export const TON_API_TOKEN =
    'AEOBPSNAUIB7IZAAAAAJ4WOBCSRCBKM2IUD2BKZPBEPKOW4XUMDLCIRGPWCJYSTSOCKUMGI';
export const TON_API_BASE_URL = 'https://testnet.tonapi.io';
export const TON_VIEWER_URL = 'https://testnet.tonviewer.com/';
export const TON_VIEWER_TRANSACTION_URL =
    'https://testnet.tonviewer.com/transaction/';

// MARK: Admin
export const ADMIN_ADDRESS = 'tb1qhsdqmnet5tagp900ww6v4dx95juaz47c7r2yep';
export const ADMIN_TON_ADDRESS =
    'UQBUKVG1AuYJuB92vDKbWu8cw9nP_UEyIczYBHU7USGmyKcs';
export const ADMIN_FEE_PERCENT = 1 / 100;

// MARK: Explorer url
export const BNB_EXPLORER_URL = 'https://testnet.bscscan.com';
export const ETH_EXPLORER_URL = 'https://etherscan.io/';
export const POL_EXPLORER_URL = 'https://amoy.polygonscan.com/';
export const TON_NON_BOUNCEABLE_MESSAGE_URL =
    'https://docs.ton.org/v3/documentation/smart-contracts/message-management/non-bounceable-messages';

// MARK: Moralis
export const MORALIS_URL = 'https://deep-index.moralis.io/api/v2.2';
export const MORALIS_API_KEY =
    'eeyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJub25jZSI6ImEwNmUzMWZiLTcyYTUtNDFlNy1iYzllLTA3NDMyODk0ZmEzMiIsIm9yZ0lkIjoiNDg4OTk1IiwidXNlcklkIjoiNTAzMTE0IiwidHlwZUlkIjoiNDIwYTBkNWItYzZkMS00OGQzLWEyNjEtMmJkYjJmM2RhM2JkIiwidHlwZSI6IlBST0pFQ1QiLCJpYXQiOjE3Njc3MTIyOTQsImV4cCI6NDkyMzQ3MjI5NH0.I1yWyiMfyRh5V4EOpvariGjTuQZfrwrwIpfVcqIC1xI';

// MARK: ChangeNow
export const CHANGE_NOW_BASE_URL = 'https://api.changenow.io/v2';
export const CHANGE_NOW_API_KEY =
    'c0e041ecf7715b8d3b8b92fa254c372ff671ca9be493244d79cb855a23b88617';
export const CHANGE_NOW_PARTNER_KEY =
    '00d7737da93e65b5b1006a58de18385c83a422e460f2536423da1d2725e3a749';
export const PINATA_API_KEY = 'your-pinata-api-key-here';
export const PINATA_API_SECRET = 'your-pinata-api-secret-here';
export const TON_API_KEY = 'your-ton-api-key-here';
export const WALLET_CONNECT_PROJECT_ID = 'defce3a32e3413b628acdf43e351afed';
export const ANDROID_APP_ID = 'com.phongvuanh772.CryptoVault';
export const IOS_APP_ID = 'com.phongvuanh772.CryptoVault';
export const SUPABASE_URL =
    process.env.EXPO_PUBLIC_SUPABASE_URL ||
    process.env.NEXT_PUBLIC_SUPABASE_URL ||
    'https://zmsgkyiqikhtwwsjhoxn.supabase.co';
export const SUPABASE_ANON_KEY =
    process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY ||
    process.env.EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY ||
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ||
    process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ||
    'sb_publishable_oWKTV0ViCo_8TTsBITbJiA_VfXziOtN';
export const NFT_COLLECTION_ADDRESS = '';
export const NFT_COLLECTION_ADDRESS_TESTNET = '';
